...
with pyaedt.Desktop(specified_version='241',non_graphical=True):
    for j in range(8000):
        ...
        positonlist, portnames = createVariation(hfss)
        hfss.analyze(cores=8)
        for i in range(17):
            
            if i == 0:
                positionDistribution = (0,0,positonlist[i+1],positonlist[i+2]-positonlist[i+1])
            elif i == 1:
                positionDistribution = (0,positonlist[i],positonlist[i+1]-positonlist[i],positonlist[i+2]-positonlist[i+1])
            elif i == 15:
                positionDistribution = (positonlist[i-1]-positonlist[i-2],positonlist[i]-positonlist[i-1],positonlist[i+1]-positonlist[i],0)
            elif i == 16:
                positionDistribution = (positonlist[i-1]-positonlist[i-2],positonlist[i]-positonlist[i-1],0,0)
            else:
                positionDistribution = (positonlist[i-1]-positonlist[i-2],positonlist[i]-positonlist[i-1],positonlist[i+1]-positonlist[i],positonlist[i+2]-positonlist[i+1])

            ccs = hfss.modeler.create_coordinate_system(origin=[positonlist[i], 0, 0], reference_cs='Global')
            ...
            rEPhiDic[positionDistribution] = pyaedt.modules.solutions.FfdSolutionData(frequencies=data.frequencies, eep_files=data.eep_files)._raw_data[f'{portnames[i]}_1']["rEPhi"]
            All17PositionDic.setdefault(i,[]).append(positionDistribution)

            dict_to_file(rEPhiDic,'F:\\pythontxtfile\\eEPhi.txt')
            dict_to_file(All17PositionDic,'F:\\pythontxtfile\\All17Position.txt')
