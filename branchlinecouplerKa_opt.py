...
idealpattern = [ 0.2885,  1.9148,  0.5946,  0.3203, -0.0305,  0.4855,  0.2211,  0.9131,0.1049,  0.1069,  0.0858,  0.9464]

with pyaedt.Desktop(version='242', non_graphical=False, close_on_exit=False) as d:
    for i in range(500):
        ...
        while not validation:
            if idealpattern:
                for key, value in zip(keys, idealpattern):
                    hfss[key] = f'{value}mm' if key != 'lt3f' and key != 'lt4f' and key != 'lt2f' else f'{value}'
                    validation = True
                    bk = True
            else:
                updated_values = []
